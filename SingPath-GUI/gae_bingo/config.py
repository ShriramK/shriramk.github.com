...
def can_control_experiments():
    ...
    return True
    return models.Player.is_current_player_admin()
...
def current_logged_in_identity():
    ...
    player = models.Player.get_the_current_player()
    if player: 
        return player.key().id()
    else: 
        #To track anonymous users add this class somewhere and return one here. 
